...
function WaterLevelCard({
  ...
  level: WaterLevel
  stats: { currentStats: USGSStats | null } | undefined
}) {
    ...
    <div className="min-h-screen bg-[#faf3e0] py-8 px-4">
      <header className="py-4 mb-8">
        <div className="max-w-6xl mx-auto px-4">
          <h1 className="text-2xl font-semibold text-center text-[#524343]">
      ...
      <div className="max-w-6xl mx-auto">
        <div
          className={`mb-6 p-4 rounded-lg text-center text-sm font-medium border border-[#c5a898] ${
        ...
        >
          ...
          {allReadingsBelowAverage ? 'below' : 'above'} historical averages
          <span className="block text-xs mt-1 opacity-75">
            Last updated:{' '}
            {waterLevels.length > 0
              ? new Date(
                  Math.max(
                    ...waterLevels.map((l) => new Date(l.dateTime).getTime())
                  )
                ).toLocaleString()
              : 'Loading...'}
          </span>
        ...
        {isLoading ? (
          <div className="text-center py-12">
            <div className="animate-pulse text-[#96624e]">
